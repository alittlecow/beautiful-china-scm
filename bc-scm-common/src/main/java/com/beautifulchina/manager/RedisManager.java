    ...
    public long incr(final String key) throws RedisException {
        ...
        try {
            result = template.execute(new RedisCallback<Long>() {
                @Override
                public Long doInRedis(RedisConnection redisConnection) {
                    long res = ((Jedis) redisConnection.getNativeConnection()).incr(key);
                    logger.debug("对redis进行增加操作异常 key={},result={}", key, res);
                    return res;
                }
            });
        } catch (Exception e) {
            logger.error("对redis进行增加操作异常，请检查相关配置。错误：{}", e.getMessage());
            throw new RedisException(e, BaseErrorCode.REDIS_CONN_EXCEPTION);
        }
        return result;
    }
